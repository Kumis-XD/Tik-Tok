document.getElementById('stalkForm').addEventListener('submit', async (e) => {
  ...
  try {
    ...
    if (data.status) {
      ...
      const posts = user.posts.slice(0, 3); // Batasi ke 3 post terbaru
      ...
      userInfoDiv.innerHTML = `
        ...
        <div class="grid grid-cols-2 gap-4 mb-4">
          <div>
            ...
            <p class="text-lg font-semibold text-gray-800 dark:text-white">${user.followers_count}</p>
          </div>
          <div>
            <p class="text-sm font-medium text-gray-700 dark:text-gray-300">Mengikuti</p>
            <p class="text-lg font-semibold text-gray-800 dark:text-white">${user.following_count}</p>
          </div>
          <div>
            <p class="text-sm font-medium text-gray-700 dark:text-gray-300">Postingan</p>
            <p class="text-lg font-semibold text-gray-800 dark:text-white">${user.posts_count}</p>
          </div>
          <div>
            <p class="text-sm font-medium text-gray-700 dark:text-gray-300">Akun Bisnis</p>
            <p class="text-lg font-semibold text-gray-800 dark:text-white">${user.is_business_account ? 'Ya' : 'Tidak'}</p>
          </div>
        </div>
        <div class="border-t border-gray-200 dark:border-gray-700 pt-4">
          ...
          <ul class="space-y-2 text-sm text-gray-600 dark:text-gray-400">
            <li>
              <span class="font-medium">Link Eksternal:</span>
              ${user.external_url ? `<a href="${user.external_url}" target="_blank" class="text-indigo-600 dark:text-indigo-400 hover:underline">${user.external_url}</a>` : 'Tidak ada'}
            </li>
          </ul>
        </div>
        <div class="border-t border-gray-200 dark:border-gray-700 pt-4">
